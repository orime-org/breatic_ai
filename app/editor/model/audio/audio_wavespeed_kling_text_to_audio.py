...
class AudioWavespeedKlingTextToAudio(BaseModel):
    """
    Wavespeed Kling 文本到语音模型
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/kwaivgi/kling-text-to-audio",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    ...
                    resp_data = resp.json()
                    logging.info(f"AudioWavespeedKlingTextToAudio get_generate_result response: {resp_data}")
                ...
                logging.error(f"AudioWavespeedKlingTextToAudio get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"AudioWavespeedKlingTextToAudio get_generate_result error: {e}")

audio_wavespeed_kling_text_to_audio_instance = AudioWavespeedKlingTextToAudio()
