...
class AudioWavespeedMinimaxMusic2(BaseModel):
    """
    Minimax Music 02 音乐生成模型
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        payload = {
            "bitrate": 256000,
            "lyrics": messages,
            "prompt": kwargs.get("music_settings", ""),
            "sample_rate": 44100
        }
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/minimax/music-02",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    ...
                    resp_data = resp.json()
                    logging.info(f"AudioWavespeedMinimaxMusic2 get_generate_result response: {resp_data}")
                ...
                logging.error(f"AudioWavespeedMinimaxMusic2 get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"AudioWavespeedMinimaxMusic2 get_generate_result error: {e}")

audio_wavespeed_minimax_music_2_instance = AudioWavespeedMinimaxMusic2()
