...
class ImageWavespeedIdeogramV3Quality(BaseModel):
    """
    图片生成模型 Ideogram V3 Quality
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/ideogram-ai/ideogram-v3-quality",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    logging.info(f"ImageWavespeedIdeogramV3Quality get_generate_result response: {generate_result}")
                ...
                logging.error(f"ImageWavespeedIdeogramV3Quality get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"ImageWavespeedIdeogramV3Quality get_generate_result error: {e}")

image_wavespeed_ideogram_v3_quality_instance = ImageWavespeedIdeogramV3Quality()
