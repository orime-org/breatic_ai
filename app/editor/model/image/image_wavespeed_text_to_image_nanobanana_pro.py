...
class ImageWavespeedTextToImageNanobananaPro(BaseModel):
    """
    图片生成模型
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/google/nano-banana-pro/text-to-image",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    logging.info(f"ImageWavespeedTextToImageNanobananaPro get_generate_result response: {generate_result}")
                ...
                logging.error(f"ImageWavespeedTextToImageNanobananaPro get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"ImageWavespeedTextToImageNanobananaPro get_generate_result error: {e}")

image_wavespeed_text_to_image_nanobanana_pro_instance = ImageWavespeedTextToImageNanobananaPro()
