...
class ImageWavespeedImageToImageFluxKontextPro(BaseModel):
    """
    图片生成模型
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/wavespeed-ai/flux-kontext-pro",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    logging.info(f"ImageWavespeedImageToImageFluxKontextPro get_generate_result response: {generate_result}")
                ...
                logging.error(f"ImageWavespeedImageToImageFluxKontextPro get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"ImageWavespeedImageToImageFluxKontextPro get_generate_result error: {e}")

image_wavespeed_image_to_image_flux_kontext_pro_instance = ImageWavespeedImageToImageFluxKontextPro()
