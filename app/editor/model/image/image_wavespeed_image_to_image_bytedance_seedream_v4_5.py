...
class ImageWavespeedImageToImageBytedanceSeedreamV45(BaseModel):
    """
    图片生成模型 Bytedance Seedream V4.5
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/bytedance/seedream-v4.5/edit",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    logging.info(f"ImageWavespeedImageToImageBytedanceSeedreamV45 get_generate_result response: {generate_result}")
                ...
                logging.error(f"ImageWavespeedImageToImageBytedanceSeedreamV45 get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"ImageWavespeedImageToImageBytedanceSeedreamV45 get_generate_result error: {e}")

image_wavespeed_image_to_image_bytedance_seedream_v4_5_instance = ImageWavespeedImageToImageBytedanceSeedreamV45()
