...
class ImageWavespeedImageToImageNanobananaPro(BaseModel):
    """
    图片生成模型
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/google/nano-banana-pro/edit",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    logging.info(f"ImageWavespeedImageToImageNanobananaPro get_generate_result response: {generate_result}")
                ...
                logging.error(f"ImageWavespeedImageToImageNanobananaPro get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"ImageWavespeedImageToImageNanobananaPro get_generate_result error: {e}")

image_wavespeed_image_to_image_nanobanana_pro_instance = ImageWavespeedImageToImageNanobananaPro()
