...
class ImageWavespeedImageToImageGptImage1(BaseModel):
    """
    图片生成模型
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/openai/gpt-image-1",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    logging.info(f"ImageWavespeedImageToImageGptImage1 get_generate_result response: {generate_result}")
                ...
                logging.error(f"ImageWavespeedImageToImageGptImage1 get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"ImageWavespeedImageToImageGptImage1 get_generate_result error: {e}")

image_wavespeed_image_to_image_gpt_image_1_instance = ImageWavespeedImageToImageGptImage1()