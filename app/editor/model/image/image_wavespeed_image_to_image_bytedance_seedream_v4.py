...
class ImageWavespeedImageToImageBytedanceSeedreamV4(BaseModel):
    """
    图片生成模型 Bytedance Seedream V4
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/bytedance/seedream-v4/edit",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    logging.info(f"ImageWavespeedImageToImageBytedanceSeedreamV4 get_generate_result response: {generate_result}")
                ...
                logging.error(f"ImageWavespeedImageToImageBytedanceSeedreamV4 get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"ImageWavespeedImageToImageBytedanceSeedreamV4 get_generate_result error: {e}")

image_wavespeed_image_to_image_bytedance_seedream_v4_instance = ImageWavespeedImageToImageBytedanceSeedreamV4()
