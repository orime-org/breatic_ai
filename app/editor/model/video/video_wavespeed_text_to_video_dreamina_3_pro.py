...
class VideoWavespeedTextToVideoDreamina3Pro(BaseModel):
    """
    Dreamina 3 Pro 视频生成模型
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/bytedance/dreamina-v3.0-pro/text-to-video",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    ...
                    resp_data = resp.json()
                    logging.info(f"VideoWavespeedTextToVideoDreamina3Pro get_generate_result response: {resp_data}")
                    ...
                    logging.error(f"VideoWavespeedTextToVideoDreamina3Pro get_generate_result status fail: {generate_result["error"]}")
                    raise ModelError(BizCode.MODEL_STATUS_FAILED.code, generate_result["error"])
            except ModelError as e:
                raise e
            except Exception as e:
                logging.error(f"VideoWavespeedTextToVideoDreamina3Pro get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"VideoWavespeedTextToVideoDreamina3Pro get_generate_result error: {e}")

video_wavespeed_text_to_video_dreamina_3_pro_instance = VideoWavespeedTextToVideoDreamina3Pro()
