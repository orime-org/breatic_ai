...
class VideoWavespeedLipSync(BaseModel):
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        payload = {
            "model": "lipsync-2",
            "video": messages,
            "audio": kwargs.get("source_audio",""),
            "sync_mode": kwargs.get("cut_video_model",""),
        }
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/sync/lipsync-2",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    logging.info(f"VideoWavespeedLipSync get_generate_result response: {generate_result}")
                    ...
                    logging.error(f"VideoWavespeedLipSync get_generate_result status fail: {generate_result["error"]}")
                    raise ModelError(BizCode.MODEL_STATUS_FAILED.code, generate_result["error"])
            except ModelError as e:
                raise e
            except Exception as e:
                logging.error(f"VideoWavespeedLipSync get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"VideoWavespeedLipSync get_generate_result error: {e}")

video_wavespeed_lip_sync_instance = VideoWavespeedLipSync()