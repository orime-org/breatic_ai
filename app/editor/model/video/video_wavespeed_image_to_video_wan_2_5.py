...
class VideoWavespeedImageToVideoWan25(BaseModel):
    """
    万相2.5 视频生成模型
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/alibaba/wan-2.5/image-to-video",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    ...
                    resp_data = resp.json()
                    logging.info(f"VideoWavespeedImageToVideoWan25 get_generate_result response: {resp_data}")
                    ...
                    logging.error(f"VideoWavespeedImageToVideoWan25 get_generate_result status fail: {generate_result["error"]}")
                    raise ModelError(BizCode.MODEL_STATUS_FAILED.code, generate_result["error"])
            except ModelError as e:
                raise e
            except Exception as e:
                logging.error(f"VideoWavespeedImageToVideoWan25 get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"VideoWavespeedImageToVideoWan25 get_generate_result error: {e}")

video_wavespeed_image_to_video_wan_2_5_instance = VideoWavespeedImageToVideoWan25()
