...
class VideoWavespeedTextToVideoKling25TurboPro(BaseModel):
    """
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/kwaivgi/kling-v2.5-turbo-pro/text-to-video",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    ...
                    resp_data = resp.json()
                    logging.info(f"VideoWavespeedTextToVideoKling25TurboPro get_video response: {resp_data}")
                    ...
                    logging.error(f"VideoWavespeedTextToVideoKling25TurboPro get_generate_result status fail: {generate_result["error"]}")
                    raise ModelError(BizCode.MODEL_STATUS_FAILED.code, generate_result["error"])
                
            except ModelError as e:
                raise e
            except Exception as e:
                logging.error(f"VideoWavespeedTextToVideoKling25TurboPro get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"VideoWavespeedTextToVideoKling25TurboPro get_generate_result error: {e}")

video_wavespeed_text_to_video_kling_2_5_turbo_pro_instance = VideoWavespeedTextToVideoKling25TurboPro()
