...
class VideoWavespeedImageToVideoBytedanceDreamina3Pro(BaseModel):
    """
    字节梦3 Pro 视频生成模型
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/bytedance/dreamina-v3.0-pro/image-to-video",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    ...
                    resp_data = resp.json()
                    logging.info(f"VideoWavespeedImageToVideoBytedanceDreamina3Pro get_generate_result response: {resp_data}")
                    ...
                    logging.error(f"VideoWavespeedImageToVideoBytedanceDreamina3Pro get_generate_result status fail: {generate_result["error"]}")
                    raise ModelError(BizCode.MODEL_STATUS_FAILED.code, generate_result["error"])
            except ModelError as e:
                raise e
            except Exception as e:
                logging.error(f"VideoWavespeedImageToVideoBytedanceDreamina3Pro get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"VideoWavespeedImageToVideoBytedanceDreamina3Pro get_generate_result error: {e}")

video_wavespeed_image_to_video_bytedance_dreamina_3_pro_instance = VideoWavespeedImageToVideoBytedanceDreamina3Pro()
