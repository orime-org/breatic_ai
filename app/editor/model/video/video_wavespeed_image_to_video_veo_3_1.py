...
class VideoWavespeedImageToVideoVeo31(BaseModel):
    """
    VEO 3.1 视频生成模型
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/google/veo3.1/image-to-video",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    ...
                    resp_data = resp.json()
                    logging.info(f"VideoWavespeedImageToVideoVeo31 get_generate_result response: {resp_data}")
                    ...
                    logging.error(f"VideoWavespeedImageToVideoVeo31 get_generate_result status fail: {generate_result["error"]}")
                    raise ModelError(BizCode.MODEL_STATUS_FAILED.code, generate_result["error"])
            except ModelError as e:
                raise e
            except Exception as e:
                logging.error(f"VideoWavespeedImageToVideoVeo31 get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"VideoWavespeedImageToVideoVeo31 get_generate_result error: {e}")

video_wavespeed_image_to_video_veo_3_1_instance = VideoWavespeedImageToVideoVeo31()
