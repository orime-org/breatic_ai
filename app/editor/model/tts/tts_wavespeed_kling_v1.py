...
class TTSWavespeedKlingV1(BaseModel):
    """
    Kling V1 语音生成模型
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/kwaivgi/kling-v1-tts",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    ...
                    resp_data = resp.json()
                    logging.info(f"TTSWavespeedKlingV1 get_generate_result response: {resp_data}")
                ...
                logging.error(f"TTSWavespeedKlingV1 get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"TTSWavespeedKlingV1 get_generate_result error: {e}")

tts_wavespeed_kling_v1_instance = TTSWavespeedKlingV1()
