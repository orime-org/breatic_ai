...
class TTSWavespeedElevenlabsTurbo25(BaseModel):
    """
    Elevenlabs Turbo 2.5 语音生成模型
    """
    _instance = None
    ...
    async def acall(self, messages: Any,**kwargs) -> str:
        ...
        resp = await self.post_with_retry0(
            path="https://api.wavespeed.ai/api/v3/elevenlabs/turbo-v2.5",
        ...
        while True:
            try:
                ...
                if generate_result["status"] == "completed":
                    ...
                    resp_data = resp.json()
                    logging.info(f"TTSWavespeedElevenlabsTurbo25 get_generate_result response: {resp_data}")
                ...
                logging.error(f"TTSWavespeedElevenlabsTurbo25 get_generate_result error: {e}")
                raise ModelError(BizCode.MODEL_REQUEST_FAILED.code, f"TTSWavespeedElevenlabsTurbo25 get_generate_result error: {e}")

tts_wavespeed_elevenlabs_turbo_2_5_instance = TTSWavespeedElevenlabsTurbo25()
