...
import { Button, Select, Checkbox, Image, Tooltip, App } from "antd";
...
import { aspectRatioType, imageModelType, tipsType } from "../../Types/nodeControlType";
import { HandleType, NodeImageToImageRuntimeData, SelectedResultsType, getResultsForRunning, selectedResultInfo, useUpstreamInputs, NodeImageInputRuntimeData } from "../../Types/runtimeData";
import { NodeImageToImageResultData } from "../../Types/resultData";
import {
...
const COMPONMENT_TITLE = "Batch Image-to-Image";
function NodeImageToImage({ id, type, selected }: { id: string; type: NodeTemplateType; selected: boolean }) {
  /**
   * Store数据
   */
  //#region
  ...
  // 图片模型选项列表
  const [imageModelOptions, setImageModelOptions] = useState<imageModelType[]>([]);
  ...
  // 模型数据
  const imageModelDatas = nodeTemplateData?.content?.[CONTROL_TAG.IMAGE_MODEL] || [],
    tipsData = nodeTemplateData?.content?.[CONTROL_TAG.TIPS] || {};

  // 当前节点执行完整骨架屏数据
  const [currentNodeExecuteFullSkeletonData, setCurrentNodeExecuteFullSkeletonData] = useState<NodeImageToImageResultData[]>([]);

  // 上游数据
  const { counter, handleCounters } = useUpstreamInputs(id);
  // 解构连接桩的连接数量
  const { 0: ImageCounter, 1: TextCounter } = handleCounters;
  ...
  // 初始化数据： 比例下拉列表项数据、图片模型下拉列表项数据、提示信息
  useEffect(() => {
    const llmsModelSelectOptions = () => {
      if (!imageModelDatas || !Array.isArray(imageModelDatas)) {
        return [];
      }

      // 映射到组件使用的结构
      return imageModelDatas.map((item: any, index: number) => ({
    ...
    const aspectRatioDatas = imageModelDatas?.[imageModelIndex]?.[CONTROL_TAG.ASPECT_RATIO] || [];
    // 比例 下拉列表选项
    ...
    setImageModelOptions(llmsModelSelectOptions);
    setTips(tipsData);
  }, [imageModelDatas, tipsData]);
  ...
  useEffect(() => {
    ...
    if (!nodeRuntimeData) {
      // 新建空节点数据初始化
      let currentNodeRuntimeData: NodeImageToImageRuntimeData = {
        imageModelIndex: 0,
        aspectRatioIndex: 0,
      ...
      // 依据当前的模型索引，变更比率数据
      const aspectRatioDatas = imageModelDatas[Number(currentNodeRuntimeData.imageModelIndex)].aspect_ratio;
      const aspectRatioSelectOptions = () => {
        if (!aspectRatioDatas || !Array.isArray(aspectRatioDatas)) {
          return [];
        }
      ...
      setImageModelIndex(currentNodeRuntimeData.imageModelIndex);
    } else {
      // 节点执行结果的数据初始化
      setAspectRatioIndex(nodeRuntimeData.aspectRatioIndex);
      setImageModelIndex(nodeRuntimeData.imageModelIndex);
      setTitle(nodeRuntimeData.title);

      // 重置AspectRatio
      // 比例 下拉列表选项
      const aspectRatioDatas = imageModelDatas[Number(nodeRuntimeData.imageModelIndex)].aspect_ratio;
      const aspectRatioSelectOptions = () => {
        if (!aspectRatioDatas || !Array.isArray(aspectRatioDatas)) {
          return [];
        }
  ...
  const handleImageModelSelect = useCallback(
    (value: string) => {
      setNodeRuntimeDataByKey(id, "imageModelIndex", Number(value));
      setImageModelIndex(Number(value));

      // 重置AspectRatio
      // 比例 下拉列表选项
      const aspectRatioDatas = imageModelDatas[Number(value)].aspect_ratio;
      const aspectRatioSelectOptions = () => {
        if (!aspectRatioDatas || !Array.isArray(aspectRatioDatas)) {
          return [];
        }
  ...
  const handleNodeExecuteClickEvent = () => {
    ...
    console.log("NodeImageToImage getResultsForRunning:", sourceText, sourceImage);

    if (nodeExecuteState !== NODE_EXECUTE_STATUS.RUNNING) {
      setNodeExecuteState(NODE_EXECUTE_STATUS.RUNNING);

      // 构建骨架屏数据
      const skeletonResults = Array.from({ length: ImageCounter }, (_, i) => ({
        index: i,
        id: "",
        result: "",
        status_code: NODE_EXECUTE_STATUS_CODE.ERROR,
        source_text: sourceText[0],
        source_image: sourceImage[i],
        credits: 0,
        exec_time: 0,
        create_time: 0,
        node_exec_id: "",
        model_id: imageModelDatas[imageModelIndex].value,
        model_name: imageModelDatas[imageModelIndex].label,
        model_icon_name: imageModelDatas[imageModelIndex].icon_name,
      }));
    ...
    const aspectRatioDatas = imageModelDatas[Number(imageModelIndex)].aspect_ratio;
    ...
    const buildRequestParams = () => {
      return {
        model_id: imageModelDatas[imageModelIndex].value,
        template_code: Number(NodeTemplateType.IMAGE_TO_IMAGE),
        image_based_blocks: {
          source_text: sourceText,
          source_image: sourceImage,
          ratio: aspectRatioDatas[aspectRatioIndex].value,
        },
        id: id,
        flow_id: workflowId,
        extra_info: {
              ...
              const next = prev.map((item) => {
                if (item.index === nodeExecutionResulDataRow.exec_result.index) {
                  ...
                  return {
                    ...
                    source_image: exec.source_image ?? item.source_image ?? [],
                    ...
                    for (const item of nodeData) {
                      ...
                      for (const exec of exec_result) {
                        ...
                        nodeBatchResultData.push({
                          ...
                          source_image: exec.source_image,
  ...
  const getParentSelectableKeys = useCallback(
    (pIndex: number) => {
      ...
      const keys: string[] = [];
      nodeResultData.forEach((singleExecuteResultsData: NodeImageToImageResultData[], executeIndex: number) => {
   ...
   * @param url 图片 URL
   */
  const handleImageCopy = (url: string) => {
    if (!url) {
    ...
    const newNodeId = `${NodeTemplateType.IMAGE_INPUT}-${Date.now()}-${uuid()}`;
    const newNode = {
      id: newNodeId,
      type: NodeTemplateType.IMAGE_INPUT,
      position: basePos,
      data: { type: "input", label: "Image" },
    } as any;
                ...
                <div className="flex1 h-[44px] flex items-center hover:cursor-grab active:cursor-grabbing">
                  ...
                  <div className="ml-auto w-[16px] h-[19px] mr-[11px] flex items-center justify-center">
                    <Tooltip
                      title={
                        <div className="inline-block whitespace-nowrap max-w-none p-[10px]">
                          ...
                          {tips?.items?.map((item: string, index: number) => (
                            <div key={"NodeImageToImage_TipsSection_" + index} className="flex items-center text-[10px]">
                              <br />
                              {item || ""}
                            </div>
                          ))}
                      ...
                      classNames={{ root: "node-imagetoimage-tooltip" }}
              ...
              <div className="content-container flex flex-col p-[12px] shadow-none bg-white rounded-b-[5px]">
                ...
                <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                  ...
                  <Select
                    ...
                    value={imageModelIndex.toString()}
                    onChange={handleImageModelSelect}
                    className="node-workflow w-full"
                    classNames={{
                      popup: {
                        root: "node-imagetoimage-modals-select",
                      },
                    }}
                    size="large"
                    ...
                    labelRender={({ value }) => {
                      const data = imageModelOptions.find((o) => o.value === value);
                      if (!data) return null;
                ...
                <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                  ...
                  <Select
                    ...
                    classNames={{
                      popup: {
                        root: "node-imagetoimage-aspectratio-select",
                      },
                    }}
                    size="large"
                ...
                <div className="space-y-2 px-[10px] pt-[12px] nodrag" style={{ position: "relative", zIndex: 10 }}>
                  <div className="flex items-center justify-center w-full">
                    <div className="flex items-center flex-1 gap-[12px]">
                      <div className="flex items-center">
                        <EditorNodes_NodeExecuteTime_Svg />
                        <div className="w-auto h-[15px] text-[10px] text-[#262626] font-normal ml-[3px] flex items-center justify-center px-[7px] py-[3px] bg-zinc-100 rounded-lg">{`~ ${imageModelDatas[imageModelIndex]?.genrate_time || "?"}s`}</div>
                      </div>
                      <div className="flex items-center">
                        <EditorNodes_NodeExecuteCredit_Svg />
                        <div className="w-auto h-[15px] text-[10px] text-[#262626] font-normal ml-[3px] flex items-center justify-center px-[7px] py-[3px] bg-zinc-100 rounded-lg">{`~ ${(handleCounters?.[0] || 1) * imageModelDatas[imageModelIndex]?.credits || "?"} Credits`}</div>
                      </div>
                    </div>
                    {/* 当前节点的执行状态 */}
                        ...
                        {nodeExecuteState === NODE_EXECUTE_STATUS.RUNNING && currentNodeExecuteFullSkeletonData?.length > 0 ? (
                          <>
                            ...
                            <div className="border-box border border-[#E9E9E9] p-1 rounded-[5px]">
                              ...
                              <div className="p-1">
                                <div className="text-[10px] font-normal text-[#989898] line-clamp-2">
                                  <Tooltip title={<div className="text-2.5 inline-block p-[10px] whitespace-pre-wrap select-text cursor-text text-justify">{`${currentNodeExecuteFullSkeletonData?.[0]?.source_text || ""}`}</div>} getPopupContainer={() => anchorRef.current!} mouseEnterDelay={0.1}>
                                    <div className="w-full text-justify">{`${currentNodeExecuteFullSkeletonData?.[0]?.source_text || ""}`}</div>
                                  </Tooltip>
                                </div>
                              </div>
                              {currentNodeExecuteFullSkeletonData.map((item, sourceIndex) => {
                                return (
                                  <div key={`${id}-SkeletonTop-${sourceIndex}`} className="flex flex-col">
                                    <div
                                      className={`grid w-full ${sourceIndex !== 0 ? "mt-1" : ""}`}
                                      style={{
                                    ...
                                    >
                                      <div className="text-card-foreground overflow-hidden shadow-none border-0 rounded-none" draggable="true">
                                        <div className="p-0 relative group h-auto">
                                          ...
                                          >
                                            <div
                                              className="round-[4px] absolute top-0 left-0  w-[282px] h-[159px] cursor-pointer overflow-hidden bg-[#F3F3F3]"
                                              style={{
                                                aspectRatio: aspectRatio,
                                              }}
                                            >
                            ...
                            {nodeResultData?.[0]?.length > 0 &&
                              nodeResultData?.map((singleExecuteResultsData: NodeImageToImageResultData[], executeIndex: number) => {
                                return (
                                  <div key={`${id}-${singleExecuteResultsData?.[0]?.id}-0`} className="border-box border border-[#E9E9E9] p-1 rounded-[5px] mt-1">
                                    {/* 数据块勾选 + 查看 */}
                                    <div className="flex items-center justify-between rounded-t-[5px] p-1 w-full">
                                      <div className="flex items-center">
                                    ...
                                    <div className="p-1">
                                      <div className="text-[10px] font-normal text-[#989898] line-clamp-2">
                                        <Tooltip title={<div className="text-2.5 inline-block p-[10px] whitespace-pre-wrap select-text cursor-text text-justify">{`${singleExecuteResultsData?.[0]?.source_text || ""}`}</div>} getPopupContainer={() => anchorRef.current!} mouseEnterDelay={0.1}>
                                    ...
                                    {singleExecuteResultsData &&
                                      singleExecuteResultsData?.map((item, index: number) => {
                                        return (
                                          <div key={`${id}-${item?.id}-${index}`} className="flex flex-col">
                                            <div
                                              className={`grid w-full ${index !== 0 ? "mt-1" : ""}`}
                                              style={{
                                                gridTemplateColumns: "repeat(1, minmax(0px, 1fr))",
                                              }}
                                            >
...
export default memo(NodeImageToImage);
