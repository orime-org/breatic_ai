...
import { videoModelType, tipsType, videoFillModelType } from "../../Types/nodeControlType";
import { NodeVideoLipSyncRuntimeData, SelectedResultsType, getResultsForRunning, useUpstreamInputs, selectedResultInfo, HandleType, NodeVideoInputRuntimeData } from "../../Types/runtimeData";
import { NodeVideoLipSyncResultData } from "../../Types/resultData";
...
const COMPONMENT_TITLE = "Batch Audio Lip Sync";
function NodeVideoLipSync({ id, type, selected }: { id: string; type: NodeTemplateType; selected: boolean }) {
  /**
   * Store数据
   */
  //#region
  ...
  // 视频填充模式索引
  const [fillModelIndex, setFillModelIndex] = useState(0);
  // 视频模型索引
  const [modelIndex, setModelIndex] = useState(0);

  // 视频填充模式列表
  const [fillModelList, setFillModelList] = useState<videoFillModelType[]>([]);
  // 视频模型列表
  ...
  const fillModelDatas = nodeTemplateData?.content?.[CONTROL_TAG.VIDEO_FILL_MODEL] || [],
    modelDatas = nodeTemplateData?.content?.[CONTROL_TAG.VIDEO_MODEL] || [],
    tipsData = nodeTemplateData?.content?.[CONTROL_TAG.TIPS] || {};

  // 当前节点执行完整骨架屏数据
  const [currentNodeExecuteFullSkeletonData, setCurrentNodeExecuteFullSkeletonData] = useState<NodeVideoLipSyncResultData[]>([]);

  // 上游数据
  const { counter, handleCounters } = useUpstreamInputs(id);
  // 解构连接桩的连接数量
  const { 0: VideoCounter, 1: AudioCounter } = handleCounters;
  ...
  // 初始化数据： 视频填充模式下拉列表项数据、视频模型下拉列表项数据、提示信息
  useEffect(() => {
    ...
    const fillModelSelectOptions = () => {
      if (!fillModelDatas || !Array.isArray(fillModelDatas)) {
        return [];
      }

      // 映射到组件使用的结构
      return fillModelDatas.map((item: any, index: number) => ({
        value: index.toString(),
        label: <span>{item.label}</span>,
      }));
    };
    setFillModelList(fillModelSelectOptions);

    setTips(tipsData);
  }, [modelDatas, fillModelDatas, tipsData]);

  // 初始化数据：视频填充模式列表选中值、模型选中值
  useEffect(() => {
    const nodeRuntimeData = useStore.getState().nodeRuntimeData?.[id];
    if (!nodeRuntimeData) {
      // 新建空节点数据初始化
      let currentNodeRuntimeData: NodeVideoLipSyncRuntimeData = {
        modelIndex: 0,
        fillModelIndex: 0,
      ...
      setFillModelIndex(currentNodeRuntimeData.fillModelIndex);
      setModelIndex(currentNodeRuntimeData.modelIndex);
    } else {
      // 节点执行结果的数据初始化
      setFillModelIndex(nodeRuntimeData.fillModelIndex);
  ...
  // VideoFillModel 下拉列表选项 选择事件
  const handleFillModelSelect = useCallback(
    (value: string) => {
      setNodeRuntimeDataByKey(id, "fillModelIndex", Number(value));
      setFillModelIndex(Number(value));
    },
    [id]
  );
  ...
  const handleNodeExecuteClickEvent = () => {
    ...
    InputNodeModel?.handle?.source?.map((item, index) => {
      ...
      resultsList.forEach((group) => {
        group.forEach((item: any | string) => {
          ...
          else if (inputHandleType === HandleType.VIDEO) sourceVideo.push(value);
        });
      });
    });

    console.log("NodeVideoLipSync getResultsForRunning", sourceAudio, sourceVideo);

    if (nodeExecuteState !== NODE_EXECUTE_STATUS.RUNNING) {
      setNodeExecuteState(NODE_EXECUTE_STATUS.RUNNING);

      // 构建骨架屏数据
      const skeletonResults = Array.from({ length: VideoCounter }, (_, i) => ({
        index: i,
        id: "",
        result: "",
        status_code: NODE_EXECUTE_STATUS_CODE.ERROR,
        source_audio: sourceAudio[0],
        source_video: sourceVideo[i],
        credits: 0,
        exec_time: 0,
        create_time: 0,
        node_exec_id: "",
    ...
    const buildRequestParams = () => {
      return {
        ...
        video_based_blocks: {
          ...
          cut_video_model: fillModelDatas[fillModelIndex].value,
        },
        extra_info: {
          model_name: modelDatas[modelIndex].label,
          model_icon_name: modelDatas[modelIndex].icon_name,
    ...
    console.log("NodeVideoLipSync handleNodeExecuteClickEvent 构建的请求参数:", requestParams);
              ...
              prev.map((item) => {
                if (item.index === nodeExecutionResulDataRow.exec_result.index) {
                  ...
                  return {
                    ...
                    source_audio: exec.source_audio ?? item.source_audio ?? "",
                    source_video: exec.source_video ?? item.source_video ?? "",
                    ...
                    for (const item of nodeData) {
                      ...
                      for (const exec of exec_result) {
                        ...
                        nodeBatchResultData.push({
                          ...
                          source_audio: exec.source_audio,
                          source_video: exec.source_video,
  ...
  const getParentSelectableKeys = useCallback(
    (pIndex: number) => {
      ...
      const keys: string[] = [];
      nodeResultData.forEach((singleExecuteResultsData: NodeVideoLipSyncResultData[], executeIndex: number) => {
              ...
              <div className="w-[300px]">
                ...
                <div className="flex1 h-[44px] flex items-center hover:cursor-grab active:cursor-grabbing">
                  ...
                  <div className="ml-auto w-[16px] h-[19px] mr-[11px] flex items-center justify-center">
                    <Tooltip
                      title={
                        <div className="inline-block whitespace-nowrap p-[10px]">
                      ...
                      classNames={{ root: "node-addsoundtovideo-tooltip" }}
              ...
              <div className="content-container flex flex-col p-[12px] shadow-none bg-white rounded-b-[5px]">
                ...
                <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                  ...
                  <Select
                    ...
                    classNames={{
                      popup: {
                        root: "node-videolipsync-modals-select",
                      },
                    }}
                    size="large"
                ...
                <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                  <div className="flex items-center mt-[12px]">
                    <span className="text-[10px] font-bold ">Fill Model</span>
                  </div>
                  <Select
                    suffixIcon={<NodeSelectComponent_SuffixSvg />}
                    value={fillModelIndex.toString()}
                    onChange={handleFillModelSelect}
                    className="node-workflow w-full"
                    classNames={{
                      popup: {
                        root: "node-videolipsync-fillmode-select",
                      },
                    }}
                    size="large"
                    options={fillModelList}
                    style={{
                      height: "38px",
                    }}
                    getPopupContainer={() => anchorRef.current!}
                      ...
                      {nodeExecuteState === NODE_EXECUTE_STATUS.RUNNING && currentNodeExecuteFullSkeletonData?.length > 0 ? (
                        <>
                          ...
                          {nodeResultData?.[0]?.length > 0 &&
                            nodeResultData?.map((singleExecuteResultsData: NodeVideoLipSyncResultData[], executeIndex: number) => {
                              return (
...
export default memo(NodeVideoLipSync);
