...
import { Button, Select, Checkbox, Input, Skeleton, App, Tooltip } from "antd";
...
import { tipsType, audioModelType } from "../../Types/nodeControlType";
import { NodeGenerateMusicsRuntimeData, getResultsForRunning, useUpstreamInputs, SelectedResultsType, selectedResultInfo, HandleType, NodeAudioInputRuntimeData } from "../../Types/runtimeData";
import { NodeGenerateMusicsResultData } from "../../Types/resultData";
...
const COMPONMENT_TITLE = "Batch Generate Music";
function NodeGenerateMusics({ id, type, selected }: { id: string; type: NodeTemplateType; selected: boolean }) {
  /**
   * Store数据
   */
  //#region
  const nodeRuntimeData = useStore((s) => {
    console.log("NodeGenerateMusics Store", s);
    return s;
  });
  ...
  // 音乐设置文本框值
  const [musicSettingsTextValue, setMusicSettingsTextValue] = useState("");
  ...
  // 当前节点执行完整骨架屏数据
  const [currentNodeExecuteFullSkeletonData, setCurrentNodeExecuteFullSkeletonData] = useState<NodeGenerateMusicsResultData[]>([]);
  ...
  useEffect(() => {
    ...
    if (!nodeRuntimeData) {
      // 新建空节点数据初始化
      let currentNodeRuntimeData: NodeGenerateMusicsRuntimeData = {
        modelIndex: 0,
        musicSettings: "",
      ...
      setMusicSettingsTextValue(currentNodeRuntimeData.musicSettings);
    } else {
      // 节点执行结果的数据初始化
      setModelIndex(nodeRuntimeData.modelIndex);
      setMusicSettingsTextValue(nodeRuntimeData.musicSettings);
  ...
  useEffect(() => {
    ...
    if (nodeExecuteState !== NODE_EXECUTE_STATUS.RUNNING) {
      ...
      if (validCounterLength !== InputNodeModel.handle.source?.length || musicSettingsTextValue.trim().length <= 10 || musicSettingsTextValue.trim().length > 2000) {
        setNodeExecuteState(NODE_EXECUTE_STATUS.WAIT);
      } else {
        setNodeExecuteState(NODE_EXECUTE_STATUS.READY);
      }
    }
  }, [counter, handleCounters, nodeExecuteState, musicSettingsTextValue]);
  //#endregion
  ...
  const handleNodeExecuteClickEvent = () => {
    ...
    console.log("NodeGenerateMusic getResultsForRunning", sourceText);
    ...
    const buildRequestParams = () => {
      return {
        ...
        audio_based_blocks: {
          ...
          music_settings: musicSettingsTextValue,
        },
        extra_info: {
          model_name: modelDatas[modelIndex].label,
          model_icon_name: modelDatas[modelIndex].icon_name,
    ...
    console.log("NodeGenerateMusic handleNodeExecuteClickEvent 构建的请求参数:", requestParams);
  ...
  const getParentSelectableKeys = useCallback(
    (pIndex: number) => {
      ...
      const keys: string[] = [];
      nodeResultData.forEach((singleExecuteResultsData: NodeGenerateMusicsResultData[], executeIndex: number) => {
   ...
   * @param e 输入事件
   */
  const handleMusicSettingsTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    let value = e.target.value;
    setMusicSettingsTextValue(value);
    setNodeRuntimeDataByKey(id, "musicSettings", value);
  };
              ...
              <div className="content-container flex flex-col p-[12px] shadow-none bg-white rounded-b-[5px]">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold ">Music Settings</span>
                  </div>
                  <div
                  ...
                  >
                    <TextArea
                      placeholder="Tune, genre, melody, and related musical elements. (10–2000 characters required.)"
                      value={musicSettingsTextValue}
                      onChange={handleMusicSettingsTextChange}
                      rows={3}
                      style={{ overscrollBehavior: "contain" }}
                      className={`
                        w-full
                        font-normal text-[12px] text-[#0D0D0D]
                        text-justify
                        p-[0px]
                        nodrag
                ...
                <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                  ...
                  <Select
                    ...
                    classNames={{
                      popup: {
                        root: "node-generatemusics-modals-select",
                      },
                    }}
                    size="large"
                      ...
                      {nodeExecuteState === NODE_EXECUTE_STATUS.RUNNING && currentNodeExecuteFullSkeletonData?.length > 0 ? (
                        <>
                          ...
                          {nodeResultData?.[0]?.length > 0 &&
                            nodeResultData?.map((singleExecuteResultsData: NodeGenerateMusicsResultData[], executeIndex: number) => {
                              return (
...
export default memo(NodeGenerateMusics);
