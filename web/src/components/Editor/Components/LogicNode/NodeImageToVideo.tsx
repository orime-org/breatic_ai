...
import { Button, Select, Checkbox, App, Tooltip } from "antd";
...
import { aspectRatioType, videoModelType, tipsType, durationType, resolutionType } from "../../Types/nodeControlType";
import { HandleType, NodeImageToVideoRuntimeData, NodeVideoInputRuntimeData, SelectedResultsType, getResultsForRunning, selectedResultInfo, useUpstreamInputs } from "../../Types/runtimeData";
import { NodeImageToVideoResultData } from "../../Types/resultData";
...
const COMPONMENT_TITLE = "Batch Image-to-Video";
function NodeImageToVideo({ id, type, selected }: { id: string; type: NodeTemplateType; selected: boolean }) {
  /**
   * Store数据
   */
  //#region
  ...
  // 分辨率列表
  const [resolutionList, setResolutionList] = useState<resolutionType[]>([]);
  ...
  // 当前节点执行完整骨架屏数据
  const [currentNodeExecuteFullSkeletonData, setCurrentNodeExecuteFullSkeletonData] = useState<NodeImageToVideoResultData[]>([]);
  ...
  useEffect(() => {
    ...
    if (!nodeRuntimeData) {
      // 新建空节点数据初始化
      const currentNodeRuntimeData: NodeImageToVideoRuntimeData = {
        modelIndex: 0,
        aspectRatioIndex: 0,
        durationIndex: 0,
        resolutionIndex: 0,
      ...
      // 依据当前的模型索引，变更分辨率数据
      const resolutionDatas = modelDatas[Number(currentNodeRuntimeData.modelIndex)].resolution;
      const resolutionSelectOptions = () => {
        if (!resolutionDatas || !Array.isArray(resolutionDatas)) {
          return [];
        }

        // 映射到组件使用的结构
        return resolutionDatas.map((item: any, index: number) => ({
          value: index.toString(),
          label: item.label,
        }));
      };
      setResolutionList(resolutionSelectOptions);

      setAspectRatioIndex(currentNodeRuntimeData.aspectRatioIndex);
      setDurationIndex(currentNodeRuntimeData.durationIndex);
      setResolutionIndex(currentNodeRuntimeData.resolutionIndex || 0);
      ...
      setResolutionIndex(nodeRuntimeData.resolutionIndex || 0);
      ...
      // 重置Resolution
      const resolutionDatas = modelDatas[Number(nodeRuntimeData.modelIndex)].resolution;
      const resolutionSelectOptions = () => {
        if (!resolutionDatas || !Array.isArray(resolutionDatas)) {
          return [];
        }
  ...
  const handleModelSelect = useCallback(
    (value: string) => {
      ...
      // 重置Resolution
      const resolutionDatas = modelDatas[Number(value)].resolution;
      const resolutionSelectOptions = () => {
        if (!resolutionDatas || !Array.isArray(resolutionDatas)) {
          return [];
        }
      ...
      setNodeRuntimeDataByKey(id, "resolutionIndex", 0);
    },
    [id]
  );
  ...
  // Resolution 下拉列表选项 选择事件
  const handleResolutionSelect = useCallback(
    (value: string, _options: any) => {
      setNodeRuntimeDataByKey(id, "resolutionIndex", Number(value));
      setResolutionIndex(Number(value));
    },
    [id]
  );
  ...
  const handleNodeExecuteClickEvent = () => {
    ...
    InputNodeModel?.handle?.source?.map((item, index) => {
      ...
      resultsList.forEach((group) => {
        group.forEach((item: any | string) => {
          ...
          else if (inputHandleType === HandleType.IMAGE) sourceImage.push(value);
        });
      });
    });

    console.log("NodeImageToVideo getResultsForRunning", sourceText, sourceImage);
    ...
    // 获取Resolution值
    const resolutionDatas = modelDatas[Number(modelIndex)]?.resolution;
    ...
    const buildRequestParams = () => {
      ...
      if (resolutionDatas) currentRequestParams.video_based_blocks.resolution = resolutionDatas[resolutionIndex]?.value;
    ...
    console.log("NodeImageToVideo handleNodeExecuteClickEvent 构建的请求参数:", requestParams);
    ...
    breaticFetchEventSourceWithAuth(`/api/workflow/node/sse?workflow_id=${workflowId}&update_token=${updateToken}`, {
      ...
      onmessage: (ev) => {
        console.log("NodeImageToVideo handleNodeExecuteClickEvent SSE 消息:", ev);
        try {
          if (ev.event === "abort") {
            stopReason = "abort";
  ...
  const getParentSelectableKeys = useCallback(
    (pIndex: number) => {
      ...
      const keys: string[] = [];
      nodeResultData.forEach((singleExecuteResultsData: NodeImageToVideoResultData[], executeIndex: number) => {
  ...
  const syncSelectedStatesToStore = (states: Record<string, boolean>) => {
    ...
    console.log("NodeTextToImage syncSelectedStatesToStore", transformedSelectedResults);
                ...
                <div className="flex1 h-[44px] flex items-center hover:cursor-grab active:cursor-grabbing">
                  ...
                  <div className="ml-auto w-[16px] h-[19px] mr-[11px] flex items-center justify-center">
                    <Tooltip
                      ...
                      classNames={{ root: "node-imagetovideo-tooltip" }}
              ...
              <div className="content-container flex flex-col p-[12px] shadow-none bg-white rounded-b-[5px]">
                ...
                <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                  ...
                  <Select
                    ...
                    classNames={{
                      popup: {
                        root: "node-imagetovideo-modals-select",
                      },
                    }}
                    size="large"
                ...
                {aspectRatioList.length > 0 && (
                  <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                    ...
                    <Select
                      ...
                      classNames={{
                        popup: {
                          root: "node-imagetovideo-aspectratio-select",
                        },
                      }}
                      size="large"
                ...
                {resolutionList.length > 0 && (
                  <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                    ...
                    <Select
                      suffixIcon={<NodeSelectComponent_SuffixSvg />}
                      value={resolutionIndex.toString()}
                      onChange={handleResolutionSelect}
                      className="node-workflow w-full"
                      classNames={{
                        popup: {
                          root: "node-imagetovideo-resolution-select",
                        },
                      }}
                      size="large"
                ...
                {durationList.length > 0 && (
                  <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                    ...
                    <Select
                      ...
                      classNames={{
                        popup: {
                          root: "node-imagetovideo-duration-select",
                        },
                      }}
                      size="large"
                      ...
                      {nodeExecuteState === NODE_EXECUTE_STATUS.RUNNING && currentNodeExecuteFullSkeletonData?.length > 0 ? (
                        <>
                          ...
                          {nodeResultData?.[0]?.length > 0 &&
                            nodeResultData?.map((singleExecuteResultsData: NodeImageToVideoResultData[], executeIndex: number) => {
                              return (
                        ...
                        nodeResultData?.map((singleExecuteResultsData: NodeImageToVideoResultData[], executeIndex: number) => {
                          return (
                            <div key={`${id}-${singleExecuteResultsData?.[0]?.id}-${executeIndex}`} className={`border-box border border-[#E9E9E9] p-1 rounded-[5px] ${executeIndex === 0 ? "" : "mt-1"}`}>
                              ...
                              {singleExecuteResultsData &&
                                singleExecuteResultsData?.map((item: any, index: number) => {
                                  return (
                                    <div key={`${id}-${item?.id}-${index}`} className="flex flex-col">
                                      <div
                                        className={`grid w-full ${index !== 0 ? "mt-1" : ""}`}
...
export default memo(NodeImageToVideo);
