/**
 * 文生图（Text to image）节点组件
 * 全选 Key的格式：执行数据块的逆序索引_数据行的正序索引，用于UI同步
 */
...
import { Button, Select, Checkbox, Image, App, Tooltip } from "antd";
...
import { NodeTextToImageRuntimeData, getResultsForRunning, useUpstreamInputs, SelectedResultsType, selectedResultInfo, HandleType, NodeImageInputRuntimeData } from "../../Types/runtimeData";
import { NodeTextToImageResultData } from "../../Types/resultData";
import {
...
const COMPONMENT_TITLE = "Batch Text-to-Image";
function NodeTextToImage({ id, type, selected }: { id: string; type: NodeTemplateType; selected: boolean }) {
  /**
   * Store数据
   */
  //#region
  ...
  // 图片模型选项列表
  const [modelOptions, setModelOptions] = useState<imageModelType[]>([]);
  ...
  // 当前节点执行完整骨架屏数据
  const [currentNodeExecuteFullSkeletonData, setCurrentNodeExecuteFullSkeletonData] = useState<NodeTextToImageResultData[]>([]);
  ...
  useEffect(() => {
    ...
    if (!nodeRuntimeData) {
      // 新建空节点数据初始化
      let currentNodeRuntimeData: NodeTextToImageRuntimeData = {
        modelIndex: 0,
        aspectRatioIndex: 0,
  ...
  const handleNodeExecuteClickEvent = () => {
    ...
    console.log("NodeTextToImage getResultsForRunning", sourceText);
    ...
    console.log("NodeTextToImage handleNodeExecuteClickEvent 构建的请求参数:", requestParams);
  ...
  const getParentSelectableKeys = useCallback(
    (pIndex: number) => {
      ...
      const keys: string[] = [];
      nodeResultData.forEach((singleExecuteResultsData: NodeTextToImageResultData[], executeIndex: number) => {
              ...
              <div className="content-container flex flex-col p-[12px] shadow-none bg-white rounded-b-[5px]">
                ...
                <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                  ...
                  <Select
                    ...
                    classNames={{
                      popup: {
                        root: "node-texttoimage-modals-select",
                      },
                    }}
                    size="large"
                ...
                <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                  ...
                  <Select
                    ...
                    classNames={{
                      popup: {
                        root: "node-texttoimage-aspectratio-select",
                      },
                    }}
                    size="large"
                        ...
                        {nodeExecuteState === NODE_EXECUTE_STATUS.RUNNING && currentNodeExecuteFullSkeletonData?.length > 0 ? (
                          <>
                            ...
                            {nodeResultData?.[0]?.length > 0 &&
                              nodeResultData?.map((singleExecuteResultsData: NodeTextToImageResultData[], executeIndex: number) => {
                                return (
                                              ...
                                              {item?.status_code === NODE_EXECUTE_STATUS_CODE.SUCCESS && (
                                                <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                                  <div className="flex items-center gap-1">
                                                    <Tooltip title={<div className="flex items-center inline-block whitespace-nowrap max-w-none text-[10px] p-[10px]">Copy</div>} mouseEnterDelay={0.1} getPopupContainer={() => anchorRef.current!}>
                                                      <div
                                                        className={`h-[24px] w-[24px] rounded-[3px] !p-0 bg-[#FFFFFF] ${isCopyHovered ? " outline outline-1 outline-[#000000]" : " outline outline-1 outline-[#989898]"} ${
                                                    ...
                                                    <Tooltip title={<div className="flex items-center inline-block whitespace-nowrap max-w-none text-[10px] p-[10px]">Download</div>} mouseEnterDelay={0.1} getPopupContainer={() => anchorRef.current!}>
                                                      <div
                                                        className={`h-[24px] w-[24px] rounded-[3px] !p-0 bg-[#FFFFFF] ${isDownloadHovered ? " outline outline-1 outline-[#000000]" : " outline outline-1 outline-[#989898]"} ${
                                                          item?.status_code !== NODE_EXECUTE_STATUS_CODE.SUCCESS ? " opacity-50 cursor-not-allowed" : " cursor-pointer"
                                                        }`}
                                                        aria-disabled={item?.status_code !== NODE_EXECUTE_STATUS_CODE.SUCCESS}
                                                        onClick={(e) => {
...
export default memo(NodeTextToImage);
