/**
 * 添加声音到视频（Add Sound to Video）节点组件
 */
...
import { audioModelType, tipsType } from "../../Types/nodeControlType";
import { HandleType, NodeAddSoundToVideoRuntimeData, NodeVideoInputRuntimeData, SelectedResultsType, getResultsForRunning, selectedResultInfo, useUpstreamInputs } from "../../Types/runtimeData";
import { NodeAddSoundToVideoResultData } from "../../Types/resultData";
...
const COMPONMENT_TITLE = "Batch Add Ambinent-to-Videos";
function NodeAddSoundToVideo({ id, type, selected }: { id: string; type: NodeTemplateType; selected: boolean }) {
  /**
   * Store数据
   */
  //#region
  ...
  // 当前节点执行完整骨架屏数据
  const [currentNodeExecuteFullSkeletonData, setCurrentNodeExecuteFullSkeletonData] = useState<NodeAddSoundToVideoResultData[]>([]);

  // 上游数据
  const { counter, handleCounters } = useUpstreamInputs(id);
  // 解构连接桩的连接数量
  const { 0: VideoCounter, 1: TextCounter } = handleCounters;
  ...
  // 初始化数据： 比例下拉列表项数据、音频模型下拉列表项数据、提示信息
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!nodeRuntimeData) {
      // 新建空节点数据初始化
      let currentNodeRuntimeData: NodeAddSoundToVideoRuntimeData = {
        modelIndex: 0,
  ...
  const handleNodeExecuteClickEvent = () => {
    ...
    console.log("NodeAddSoundToVideo getResultsForRunning", sourceText, sourceVideo);
    ...
    console.log("NodeAddSoundToVideo handleNodeExecuteClickEvent 构建的请求参数:", requestParams);
  ...
  const getParentSelectableKeys = useCallback(
    (pIndex: number) => {
      ...
      const keys: string[] = [];
      nodeResultData.forEach((singleExecuteResultsData: NodeAddSoundToVideoResultData[], executeIndex: number) => {
              ...
              <div className="content-container flex flex-col p-[12px] shadow-none bg-white rounded-b-[5px]">
                ...
                <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                  ...
                  <Select
                    ...
                    classNames={{
                      popup: {
                        root: "node-addsoundtovideo-modals-select",
                      },
                    }}
                    size="large"
                      ...
                      {nodeExecuteState === NODE_EXECUTE_STATUS.RUNNING && currentNodeExecuteFullSkeletonData?.length > 0 ? (
                        <>
                          ...
                          <div className="border-box border border-[#E9E9E9] p-1 rounded-[5px]">
                            ...
                            <div className="p-1">
                              <div className="text-[10px] font-normal text-[#989898] line-clamp-2">
                                <Tooltip title={<div className="text-2.5 inline-block p-[10px] select-text whitespace-pre-wrap cursor-text text-justify">{`${currentNodeExecuteFullSkeletonData?.[0]?.source_text || ""}`}</div>} getPopupContainer={() => anchorRef.current!} mouseEnterDelay={0.1}>
                          ...
                          {nodeResultData?.[0]?.length > 0 &&
                            nodeResultData?.map((singleExecuteResultsData: NodeAddSoundToVideoResultData[], executeIndex: number) => {
                              return (
...
export default memo(NodeAddSoundToVideo);
