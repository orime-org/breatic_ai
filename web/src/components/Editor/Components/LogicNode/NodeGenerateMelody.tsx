/**
 * 旋律生成 Node Generate Melody 节点组件
 */
...
import { 
  ...
  CONTROL_TAG, 
  NODE_EXECUTE_STATUS, 
  NodeExectionSSEDataGroup, 
  NODE_EXECUTE_STATUS_CODE, 
  WorkflowNodeExecType, 
  WorkflowNodeExecuteResult 
} from "../../Types/types";
import { breaticFetchEventSourceWithAuth } from "../../../../api/breaticFetch";
import dashboardApi from "../../../../api/workflowApi";
import { tipsType, audioModelType, durationType } from "../../Types/nodeControlType";
import { NodeGenerateMelodyRuntimeData, getResultsForRunning, useUpstreamInputs, SelectedResultsType, selectedResultInfo, HandleType, NodeAudioInputRuntimeData } from "../../Types/runtimeData";
import { NodeGenerateMelodyResultData } from "../../Types/resultData";
...
const COMPONMENT_TITLE = "Batch Generate Melody";
function NodeGenerateMelody({ id, type, selected }: { id: string; type: NodeTemplateType; selected: boolean }) {
  /**
   * Store数据
   */
  //#region
  ...
  // 当前节点执行完整骨架屏数据
  const [currentNodeExecuteFullSkeletonData, setCurrentNodeExecuteFullSkeletonData] = useState<NodeGenerateMelodyResultData[]>([]);
  ...
  useEffect(() => {
    ...
    if (!nodeRuntimeData) {
      // 新建空节点数据初始化
      let currentNodeRuntimeData: NodeGenerateMelodyRuntimeData = {
        modelIndex: 0,
        durationIndex: 0,
  ...
  const handleNodeExecuteClickEvent = () => {
    ...
    console.log("NodeGenerateMelody getResultsForRunning", sourceText);

    let skeletonResults = [] as NodeGenerateMelodyResultData[];

    if (nodeExecuteState !== NODE_EXECUTE_STATUS.RUNNING) {
      setNodeExecuteState(NODE_EXECUTE_STATUS.RUNNING);

      // 构建骨架屏数据
      skeletonResults = Array.from({ length: counter }, (_, i) => ({
        index: i,
        id: "",
        result: "",
    ...
    const buildRequestParams = () => {
      return {
        ...
        audio_based_blocks: {
          ...
          duration: durationDatas[durationIndex]?.value,
        },
        extra_info: {
          model_name: modelDatas[modelIndex].label,
          model_icon_name: modelDatas[modelIndex].icon_name,
    ...
    console.log("NodeGenerateMelody handleNodeExecuteClickEvent 构建的请求参数:", requestParams);
              ...
              prev.map((item, _i) => {
                if (item.index === nodeExecutionResulDataRow.exec_result.index) {
                  ...
                  return {
                    ...item,
                    id: exec.id ?? item.id ?? "",
                    result: exec.result ? (exec.result as string) : item.result,
                    status_code: isValidAudio ? NODE_EXECUTE_STATUS_CODE.SUCCESS : exec.status_code ?? item.status_code,
  ...
  const getParentSelectableKeys = useCallback(
    (pIndex: number) => {
      ...
      const keys: string[] = [];
      nodeResultData.forEach((singleExecuteResultsData: NodeGenerateMelodyResultData[], executeIndex: number) => {
              ...
              <div className="content-container flex flex-col p-[12px] shadow-none bg-white rounded-b-[5px]">
                ...
                <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                  ...
                  <Select
                    ...
                    classNames={{
                      popup: {
                        root: "node-generatemelody-modals-select",
                      },
                    }}
                    size="large"
                ...
                <div className="space-y-2 nodrag" style={{ position: "relative", zIndex: 10 }}>
                  ...
                  <Select
                    ...
                    classNames={{
                      popup: {
                        root: "node-generatemelody-duration-select",
                      },
                    }}
                    size="large"
                      ...
                      {nodeExecuteState === NODE_EXECUTE_STATUS.RUNNING && currentNodeExecuteFullSkeletonData?.length > 0 ? (
                        <>
                          ...
                          {nodeResultData?.[0]?.length > 0 &&
                            nodeResultData?.map((singleExecuteResultsData: NodeGenerateMelodyResultData[], executeIndex: number) => {
                              return (
...
export default memo(NodeGenerateMelody);
