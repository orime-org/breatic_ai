...
import { NodeVideoInputRuntimeData, selectedResultInfo, SelectedResultsType } from "../../Types/runtimeData";
...
import { EditorNodes_ImageClose_Svg, EditorNodes_TooltipSvg, Node_HandleSvg, NodeImageInput_ImageUploadSvg, EditorNodes_UploadingSpinAnimation_SVG } from "../../SvgLoader/staticIcon";
import { tipsType } from "../../Types/nodeControlType";

const COMPONMENT_TITLE = "Video";
function NodeVideoInput({ id, type, selected }: { id: string; type: NodeTemplateType; selected: boolean }) {
  /**
   * 节点运行时数据
   */
  ...
  useEffect(() => {
    ...
    if (!nodeRuntimeData) {
      const currentNodeRuntimeData: NodeVideoInputRuntimeData = {
      ...
      setTimeout(() => {
        setPreviewUrl(nodeRuntimeData.videoUrl);
  ...
  const handleFileUpload = async (file: File) => {
    try {
      ...
      messageApi.error("Video upload failed.");
      // 上传失败时清理预览
      if (previewUrl) {
        if (previewUrl.startsWith("blob:")) {
          URL.revokeObjectURL(previewUrl);
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isLt10M) {
      messageApi.error("The video file size cannot exceed 10 MB!");
      return;
    }
          ...
          <div className="content-container h-auto flex flex-col shadow-none bd-white rounded-b-[5px]">
            <div className="relative min-auto w-full overflow-visible space-y-4 p-1 bg-white rounded-b-[5px]">
              {uploading ? (
                ...
                // 2 上传成功，显示预览视频
                <div
                  className="w-full group h-[187px] flex flex-col items-center justify-center relative bg-[#F3F3F3]"
                  onClick={(e) => {
                    e.stopPropagation();
                    e.preventDefault();
                  }}
                >
                  ...
                  <Tooltip title={<div className="flex items-center inline-block whitespace-nowrap max-w-none text-[10px] p-[10px]">Remove</div>} mouseEnterDelay={0.1} getPopupContainer={() => anchorRef.current!}>
                    <div
                      className={`absolute top-1 right-1 flex items-center justify-center h-[24px] w-[24px] rounded-[3px] bg-[#FFFFFF] ${isVideoHovered ? " outline outline-1 outline-[#000000]" : " outline outline-1 outline-[#989898]"
                        } !p-0 cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity duration-200`}
                      onClick={(e) => {
                        e.stopPropagation();
                        handleRemove();
                      }}
                    ...
                    >
                      <EditorNodes_ImageClose_Svg className="block" color={isVideoHovered ? "#262626" : "#989898"} />
                    </div>
                  </Tooltip>
                </div>
              ) : (
                ...
                <div className="w-full min-h-[197px] flex flex-col items-center justify-center text-center rounded-md cursor-pointer" onClick={() => fileInputRef.current?.click()} onDragOver={handleDragOver} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDrop={handleDrop}>
                  <input type="file" ref={fileInputRef} className="hidden" accept=".mp4,.mov" onChange={handleFileSelect} />
                  <NodeImageInput_ImageUploadSvg />
                  <div className="mt-[10px]">
                    <p className="text-xs font-normal text-[#989898]">Click to select a video file</p>
                  </div>
                </div>
              )}
            </div>
...
export default memo(NodeVideoInput);
